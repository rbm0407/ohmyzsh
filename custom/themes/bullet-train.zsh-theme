...
if [ ! -n "${BULLETTRAIN_PHP_PROMPT_CMD+1}" ]; then
  BULLETTRAIN_PHP_PROMPT_CMD="\$(php -v)"
fi
...
# PHP
prompt_php() {
  if [[ (-f *.php(#qN) || -f composer.json) ]]; then
    if command -v php > /dev/null 2>&1; then
      prompt_segment $BULLETTRAIN_PHP_BG $BULLETTRAIN_PHP_FG $BULLETTRAIN_PHP_PREFIX" $(php -v | grep -Po '(?<=^PHP )[^ ]+')"
    fi
  fi
}
